...
func NewMinIOFetcher(cfg *httpserver.ObjectStorageConfig, workDir string) (*MinIOFetcher, error) {
	...
	if err := os.MkdirAll(workDir, 0o755); err != nil {
		return nil, fmt.Errorf("create preprocess workdir: %w", err)
	}
...
func (f *MinIOFetcher) Fetch(bucket, key string) (*FetchResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create local object file: %w", err)
	}
	defer file.Close()
	...
	if err != nil {
		return nil, fmt.Errorf("write object to local file: %w", err)
	}
